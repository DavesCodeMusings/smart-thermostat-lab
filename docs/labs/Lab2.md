...
## Next Steps
Now that we can measure the room's temperature, we can compare it to the temperature we're trying to maintain to decide if the heat needs to run or not. We should also have a way to adjust that temperature setting. We'll add this functionality using the ESP32's touch inputs in [Lab 3](Lab3.md).
